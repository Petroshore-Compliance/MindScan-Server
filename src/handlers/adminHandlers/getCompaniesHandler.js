const { getCompaniesController } = require("../../controllers/adminControllers/getCompaniesController.js");
...
const getCompaniesHandler = async (req, res) => {
  try {
    ...
    return res.status(response.status).json({ message: response.message, companies: response.companies });
